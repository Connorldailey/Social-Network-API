...
export const createThought = async (req, res) => {
    ...
    try {
        ...
        const user = await User.findOneAndUpdate({ _id: userId }, { $addToSet: { thoughts: thought } }, { new: true });
        if (!user) {
            res.status(404).json({
...
export const deleteThought = async (req, res) => {
    try {
        ...
        await User.findOneAndUpdate({ thoughts: thoughtId }, { $pull: { thoughts: thoughtId } }, { new: true });
        res.status(200).json({
